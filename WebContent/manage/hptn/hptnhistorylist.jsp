    ...
    <div class="box wrapper " style="overflow: auto;">
        <div class="box-header with-border">
          ...
          <div class="row">
                ...
                <div class="col-xs-1" style="min-width: 160px">
                    <button class="btn btn-block btn-default" onclick="searchhptnhistory()">
                        <i class="fa fa-search"></i>
                      	  查询
                    </button>
                </div>
                    <div class="col-xs-1 pull-right" style="min-width: 160px">
                    <button class="btn btn-block btn-primary col-sm-1" data-toggle="modal" data-target="#hptnhistory-add-modal">
...
<jsp:include page='hptnhistoryinfo.jsp'/>
<jsp:include page='hptnhistoryadd.jsp'/>
...
$("#table-list").DataTable({
    ...
    "type": "POST",
    "ajax": {
        "url": "${pageContext.request.contextPath}/manage/hptn/hptnhistory.do",
        "type": "POST",
     ...
     "columnDefs": [
        {
            ...
            "render": function (data, type, full, meta) {
                ...
                var result = '<div class="btn-group open">'
                    ...
                    + ' data-toggle="modal" data-target="#hptnhistory-info-modal">'
                    + '<i class="fa fa-gear"></i>'
                    + '</button><button class="btn btn-default btn-sm" onclick="deletehptnhistory(' + data.id + ')">'
...
// 编辑按钮 → 信息填充
 $('#hptnhistory-info-modal').on('show.bs.modal', function (event) {
     var modal = $(this);
 ...
 function edithptnhistory() {
		...
		$.post(
		    "${pageContext.request.contextPath}/manage/hptn/updateHptnHistory.do",
		    {
		        "id": id,
		        "engSnUsed": engSnUsed,
		        "hptnPn": hptnPn,
		    ...
		    function (response) {
		    	 $('#hptnhistory-info-modal').modal('hide');
		         reloadTable(true);
		    },
		    "json"
		);
 ...
 function addhptnhistory() {
		...
		$.post(
		    "${pageContext.request.contextPath}/manage/hptn/saveHptnHistory.do",
		    {
			        "engSnUsed": engSnUsed,
			        "hptnPn": hptnPn,
			        "hptnSn": hptnSn,
		    ...
		    function (response) {
		    	if(response.code != 101){
		    	 ...
		    	 $('#hptnhistory-add-modal').modal('hide');
		         reloadTable(true);
		    },
		    "json"
		);
	 ...
	 * 清除弹窗原数据
	 */
	$("#hptnhistory-add-modal").on("hidden.bs.modal", function() {
		$('#reset').click();
	});
	...
	// search 
	function searchhptnhistory() {
		reloadTable(true);
	}

	// delete
	function deletehptnhistory(id) {
		if (confirm("你确定要删除吗？")) {
			$.post(
					"${pageContext.request.contextPath}/manage/hptn/deleteHptnHistory.do",
					{
						"id" : id
					}, 
					function(response) {
